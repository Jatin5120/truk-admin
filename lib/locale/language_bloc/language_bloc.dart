import 'package:admin/sessionmanagement/session_manager.dart';
...
class LanguageBloc extends Bloc<LanguageEvent, Language> {
  ...
  Stream<Language> _mapLanguageLoadStartedToState() async* {
    ...
    Locale locale = await sharedPrefService.getLocale();

    yield Language(locale);
  }

  Stream<Language> _mapLanguageSelectedToState(
      Language selectedLanguage) async* {
    final sharedPrefService = SharedPref();
    final defaultLanguage = await sharedPrefService.getLocale();

    if (selectedLanguage.locale == SharedPref.hi &&
        defaultLanguage != SharedPref.hi) {
      yield* _loadLanguage(sharedPrefService, 'hi', 'IN');
    } else if (selectedLanguage.locale == SharedPref.en &&
        defaultLanguage != SharedPref.en) {
      yield* _loadLanguage(sharedPrefService, 'en', 'US');
    } else if (selectedLanguage.locale == SharedPref.te &&
        defaultLanguage != SharedPref.te) {
      yield* _loadLanguage(sharedPrefService, 'te', 'IN');
    }
  }

  /// This method is added to reduce code repetition.
  Stream<Language> _loadLanguage(
      SharedPref sharedPref, String languageCode, String countryCode) async* {
