...
class _ReportState extends State<Report> {
  ...
  getData() async {
    for(int i=0;i<states.length;i++){
      ...
      await FirebaseFirestore.instance.collection('FleetOwners').get().then((values){
      ...
      await FirebaseFirestore.instance.collection('Shipment').get().then((valuee) async {
        for(var w in valuee.docs){
          ...
          if (response.statusCode == 200) {
            for (var d in data['results']) {
              if(d['formatted_address'].toString().toLowerCase().contains(states[i].toLowerCase())){
                setState(() {
                  bCount+=1;
                });
                break;
    ...
    return isLoading?Container(
        child: Column(
          children: [
            ...
            Text("Generating State Wise Analytics Report Please Wait ......\n$s\n $percentage % done")
          ],
        )
    ):Container(
